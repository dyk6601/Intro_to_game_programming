...
bool Entity::hasCollidedWith(Entity *other) const
{
...
void Entity::AIFollow(Entity *target)
{
    ...
    {
    case IDLE:
        if (Vector2Distance(mPosition, target->getPosition()) < 150.0f) 
            mAIState = WALKING;
        break;
    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    bool isFlyer = (mEntityType == NPC && mAIType == FLYER);
    ...
    if (isFlyer)
    {
        // Flyers ignore vertical physics entirely.
        mVelocity.y = 0.0f;
        mIsJumping = false;
    }
    else
...
void Entity::render()
{
    ...
    switch (mTextureType)
    {
        ...
        case ATLAS:
            ...
            // `walking.png` needs half-height sampling to avoid duplicated
            // character slices. Keep single-row atlas sprites (e.g. frog)
            // unmodified.
            if (mSpriteSheetDimensions.x > 1.0f)
            {
                textureArea.y += textureArea.height / 2.0f;
                textureArea.height /= 2.0f;
            }

            // Face right by flipping source slice horizontally when moving right.
            if (mDirection == RIGHT)
            {
            ...
            if (mSpriteSheetDimensions.x > 1.0f)
                atlasVisualYOffset = mScale.y * 0.18f;
    ...
    Rectangle destinationArea = {
        mPosition.x,
        mPosition.y - atlasVisualYOffset,
