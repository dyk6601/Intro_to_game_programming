...
Map::Map(int mapColumns, int mapRows, unsigned int *levelData,
         ...
         const std::vector<unsigned int>& solidTiles) : 
         ...
         mOrigin {origin}, mSolidTiles {solidTiles.begin(), solidTiles.end()} { build(); }
...
bool Map::isSolidTileAt(Vector2 position, float *xOverlap, float *yOverlap)
{
    ...
    *yOverlap = 0.0f;
    ...
    // Legacy: if no solid list is provided, every non-zero tile is solid.
    if (!mSolidTiles.empty() && mSolidTiles.find(tile) == mSolidTiles.end())
        return false;
