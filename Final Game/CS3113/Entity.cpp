    ...
    Entity *collidableEntities, int collisionCheckCount)
{
    ...
    if (isFlyer)
    {
        // Flyers use direct movement only.
        mIsJumping = false;
    }
    else
    {
    ...
    bool shouldAnimate = (GetLength(mMovement) != 0 || mAnimateWhenStill) &&
        (isFlyer || mEntityType == PLAYER || mIsCollidingBottom);

    if (mTextureType == ATLAS && shouldAnimate)
        animate(deltaTime);
}
...
void Entity::render()
{
    ...
    switch (mTextureType)
    {
        ...
        case ATLAS:
            ...
            // For single-column atlases that reuse the same frames for both
            // directions, mirror when moving right.
            if (mSpriteSheetDimensions.x <= 1.0f && mDirection == RIGHT)
            {
    ...
    if (mUseRenderShader && mRenderShader.id > 0)
        BeginShaderMode(mRenderShader);
