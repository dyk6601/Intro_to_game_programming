...
void LevelC::initialise() {
	...
	mGameState.bgm = LoadMusicStream("assets/game/peaceful_music.mp3");
	...
	for (int i = 0; i < MAX_BULLETS; i++) {
		mBullets[i] =
				new Entity({0.0f, 0.0f}, {BULLET_SCALE, BULLET_SCALE},
									 "assets/game/All_Fire_Bullet_Pixel_16x16.png", NONE);
	...
	mGameState.xochitl = new Entity({mOrigin.x - 400.0f, mOrigin.y - 100.0f},
	...
	if (mPrincess == nullptr) {
		mPrincess = new Entity({mOrigin.x + 650.0f, mOrigin.y - 10.0f},
									  {145.0f, 145.0f},
...
void LevelC::update(float deltaTime) {
	...
	if (mHasGun && mGameState.xochitl) {
		...
		if (wantsShooting && mBulletFireCooldown <= 0.0f) {
			...
			for (int i = 0; i < MAX_BULLETS; i++) {
				if (mBullets[i] == nullptr || mBullets[i]->isActive()) continue;
	...
	if (mPrincess && mGameState.xochitl) {
		float distanceToPrincess =
			Vector2Distance(mGameState.xochitl->getPosition(), mPrincess->getPosition());
		if (distanceToPrincess <= 120.0f) {
			PlaySound(mLevelClearSound);
			mGameState.nextSceneID = LEVEL_C_TO_D_SCENE;
			return;
		}
	}
	...
	if (mSkeletonEnemy && mGameState.sharedLives &&
		...
		if (*mGameState.sharedLives <= 0) {
			...
			mPendingLifeLossSceneID = LEVEL_C_SCENE;
		}
